 ...
 * 
 * AOP 적용 단계
 * 1. Maven에 pom.xml에 AOP 의존성 추가
 * 2. Cross Cutting Concern 횡단 관심사, 공통 관심사 로직을 정의한 클래스를 구현
 * 3. AOP 설정 : xml or Annotation(Annotation으로 진행) 기반 AOP 설정 
 * 
 * AOP 적용 후 개선된 점
 * 1. 여러 서비스에서 반복적으로 작업하던 공통 작업을 AOP 별도 모듈에서 한번 정의해서 사용
 * 2. 이후 공통 작업(Cross Cutting Concern 공통 횡단 관심사 or 공통 관심사)의 업무가 변경되어도
 * 	  여러 곳을 수정 작업 필요 없이 오직 AOP 모듈만 업데이트 하면 된다
 * 	  높은 응집도 High conhesion : Core는 Core에만 집중, Cross cutting은 별도로 전담
...
public class Spring09AopSolutionApplication implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		System.out.println("**테스트를 위해 직접 객체를 생성해 호출해봄**");
